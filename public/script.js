...
import {
    ...
    setFloatingPrompt,
    shouldWIAddPrompt,
} from "./scripts/extensions/floating-prompt/index.js";
    ...
    formatting: {
        ...
        mes: [
            ...
            <li><tt>` +
                ...
                `text` +
                "```" +
                `</tt> - displays as a code block</li>
            <li><tt>` +
                "`" +
                `text` +
                "`" +
                `</tt> - displays as inline code</li>
...
$(document).ajaxError(function myErrorHandler(_, xhr) {
    if (xhr.status == 403) {
        toastr.warning(
            "doubleCsrf errors in console are NORMAL in this case. Just reload the page or close this tab.",
...
async function Generate(
    ...
    if (
        ...
        if (is_pygmalion) {
            ...
            storyString += appendToStoryString(
                charPersonality,
                power_user.disable_personality_formatting ? "" : "Personality: "
            );
            storyString += appendToStoryString(
                Scenario,
        ...
        if (main_api == "poe") {
            ...
            zeroDepthAnchor = appendPoeAnchors(
                type,
                zeroDepthAnchor,
                jailbreakPrompt
            );
        ...
        async function runGenerate(cycleGenerationPromt = "") {
            ...
            if (main_api == "openai") {
                ...
                if (isStreamingEnabled() && type !== "quiet") {
                    streamingProcessor.generator = await generatePoe(
                        type,
                        finalPromt,
                        streamingProcessor.abortController.signal
                    );
                } else {
                    generatePoe(type, finalPromt, abortController.signal)
                        .then(onSuccess)
                        .catch(onError);
                }
...
function promptItemize(itemizedPrompts, requestedMesId) {
    ...
    if (this_main_api == "openai") {
        ...
        var storyStringTokens = getTokenCount(
            itemizedPrompts[thisPromptSet].storyString
        );
        ...
        var ActualChatHistoryTokens =
            mesSendStringTokens - allAnchorsTokens + power_user.token_padding;
    ...
    if (this_main_api == "openai") {
        ...
        var storyStringTokensPercentage = (
            (oaiPromptTokens / finalPromptTokens) *
            100
        ).toFixed(2);
            ...
            jQuery.ajax({
                ...
                success: function (html) {
                    ...
                    $("#form_bg_download").after(
                        `<div class="bg_example" bgfile="${html}" style="background-image: url('${getThumbnailUrl(
                            "bg",
                            html
                        )}');">
...
function connectAPISlash(_, text) {
    ...
    if (apiConfig.source) {
        $(
            "#chat_completion_source option[value='" + apiConfig.source + "']"
        ).prop("selected", true);
